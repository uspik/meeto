...
**5. Разрешить добавлять бота в группы**

```
/mybots
```
ваш бот → **Bot Settings** → **Allow Groups?** → **Turn groups on**

Без этого бота нельзя добавить в чат, а вместе с ним не работает и группа
из чата. **Group Privacy трогать не нужно** — бот обходится служебными
сообщениями и нажатиями кнопок, читать переписку ему незачем.

Если хотите, чтобы участник выпадал из группы Meeto сразу после выхода из
супергруппы, сделайте бота администратором чата: обновление `chat_member`
Telegram шлёт только администраторам. Без прав это отработает по служебному
сообщению «вышел из чата», которое супергруппы показывают не всегда.
...
**Пересобирайте всё разом.** `api`, `bot` и `worker` собираются из одного
каталога, и если обновить только `api`, воркер останется на старом коде —
уведомления начнут расходиться с тем, что кладёт в очередь бэкенд. Сервисы
делят общий тег образа, но команда должна быть без имени сервиса:
...
**Убедиться, что новая версия действительно доехала.** Самая частая причина
«я всё обновил, а ничего не поменялось» — незакоммиченные правки или сборка
из кеша. Проверяется так:

```bash
# на сервере: коммит совпадает с тем, что вы пушили?
cd ~/meeto/app && git log --oneline -1
```

В браузере откройте `https://ваш-домен`, нажмите F12 и посмотрите консоль:
при старте mini-app печатает `Meeto build: 2026-08-09 21:59`. Время должно
быть свежим. То же значение лежит в `window.__MEETO_BUILD__`.

Если штамп старый — пересоберите без кеша:

```bash
docker compose build --no-cache miniapp && docker compose up -d miniapp
```

**Показать интерфейс в обычном браузере** (для отладки вёрстки). Telegram
подписывает вход своими данными, поэтому без него приложение показывает замок.
Обойти это можно временным токеном:

```bash
cd ~/meeto/app
echo "DEV_LOGIN_TOKEN=$(openssl rand -hex 16)" >> .env
grep DEV_LOGIN_TOKEN .env
docker compose up -d --force-recreate api
```

Дальше открывайте `https://ваш-домен/?dev=<токен>` в любом браузере — войдёте
служебным аккаунтом «Отладка». **Когда закончите, уберите строку из `.env`**
и перезапустите api: пока токен задан, войти может любой, кто его узнает.
